...
If needed, the friction factor _f_ is calculted using the Colebrook equation. The flow adjustment in each loop is calculated at each iteration as: 
$${\Delta{Q_i} = \frac{\sum_{j=1}^{p_i} r_{ij}Q_j|Q_j|}{\sum_{j=1}^{p_i} 2r_{ij}Q_j^2}}$$
where _i_ is the loop number, _j_ is the pipe number, and ${\Delta{Q_i}}$ is the flow adjustment to be applied to each pipe in loop _i_ for the next iteration.
...
Running it and looking at the output from three iterations:
